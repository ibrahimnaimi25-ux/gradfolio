            ...
            filtered.map((submission) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Score — shown prominently when present */}
                  {submission.score && effectiveStatus !== "pending" && (
                    <div className="mt-4 flex items-center gap-3 rounded-2xl border border-yellow-100 bg-yellow-50 px-4 py-3">
                      <div className="flex gap-0.5">
                        {[1,2,3,4,5].map((star) => (
                          <svg
                            key={star}
                            className={`h-5 w-5 ${star <= submission.score! ? "text-yellow-400" : "text-slate-200"}`}
                            fill="currentColor"
                            viewBox="0 0 20 20"
                          >
                  ...
                  {/* Needs revision — resubmit CTA */}
                  {effectiveStatus === "needs_revision" && (
                    <div className="mt-4 flex flex-col gap-3 rounded-2xl border border-sky-200 bg-sky-50 p-4 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        <p className="text-sm font-semibold text-sky-800">Revision required</p>
                        <p className="mt-0.5 text-xs text-sky-600">
                          Review the feedback above, then update your submission.
                        </p>
                      </div>
                      <Link
                        href={`/tasks/${submission.task_id}/submit`}
                        className="shrink-0 inline-flex items-center gap-1.5 rounded-xl bg-sky-600 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-sky-700"
                      >
                        Update submission →
                      </Link>
                    </div>
